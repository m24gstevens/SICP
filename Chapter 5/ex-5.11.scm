...
(define (make-register name stack)
    ...
    (define (dispatch message)
      (cond ((eq? message 'get) contents)
            ((eq? message 'set)
             ...
             ((eq? message 'pop-stack) (pop reg-stack))
             ((eq? message 'push-stack)
              (lambda (value) (push reg-stack value)))
            (else
...
(define (set-contents! register value)
  ((register 'set) value))
(define (pop-reg-stack register)
  (register 'pop-stack))
(define (push-reg-stack register value)
  ((register 'push-stack) value))
...
(define (make-save inst machine pc)
  (let ((reg (get-register machine
    ...
    (lambda ()
      (push-reg-stack reg (get-contents reg))
      (advance-pc pc))))
...
(define (make-restore inst machine pc)
  (let ((reg (get-register machine
                           (stack-inst-reg-name inst))))
    (lambda ()
      (set-contents! reg (pop-reg-stack reg))
      (advance-pc pc))))
