...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((quoted? exp) (object-of-quotation (text-of-quotation exp) env))
...
(define (helper quoted-list)
  (if (null? quoted-list)
      (quote '())
      (let ((first (car quoted-list))
            (rest (helper (cdr quoted-list))))
          (list 'cons
                (list 'quote first) rest))))

(define (text-of-quotation exp) (cadr exp))

(define (object-of-quotation text env)
  (if (not (pair? text))
      text
      (eval (helper text) env)))
...
; The following is credited to Felix021 from the SICP solutions wiki
; (map (lambda (name obj) 
